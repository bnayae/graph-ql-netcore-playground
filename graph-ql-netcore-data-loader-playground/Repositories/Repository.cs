...
namespace Bnaya.Samples.Repositories
{
    public class Repository : IRepository
    {
        ...
        public async Task<IDictionary<int, User>> GetUserByIdsAsync(IEnumerable<int> ids)
        {
            var tasks = from id in ids
                        select GetUserByIdAsync(id); // can be optimized when the storage / service support batch operations
            User[] users = await Task.WhenAll(tasks).ConfigureAwait(false);
            return users.ToDictionary(m => m.Id);
        }
